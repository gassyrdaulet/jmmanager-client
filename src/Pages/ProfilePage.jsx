...
export default function ProfilePage() {
  ...
  const _payOff = async (sign) => {
    ...
    try {
      ...
      getAllUsers();
      getTransactions(
        localStorage.getItem("firstDateTr"),
        localStorage.getItem("secondDateTr")
      );
      getTransactionsOther(
        localStorage.getItem("firstDateTrOther"),
        localStorage.getItem("secondDateTrOther")
      );
  ...
  const getTransactions = async (firstDate, secondDate) => {
    setIsLoading(true);
    try {
      const response = await Auth.getTransactions(
  ...
  const sortedTransactioinsOther = useMemo(() => {
    try {
      const sortedArray = [...transactionsOther].sort((a, b) =>
        b.date < a.date ? -1 : b.date > a.date ? 1 : 0
      );
      return sortedArray;
    } catch (e) {
  ...
  const currentData = sortedTransactioins.slice(
    indexOfFirstPost,
    indexOfLastPost
  );
  ...
  const currentData2 = sortedTransactioinsOther.slice(
    indexOfFirstPost2,
    indexOfLastPost2
  );
  ...
  const filterUsers = useMemo(() => {
    try {
      const temp = [...sortedUsers].filter((user) =>
        user.name.toLowerCase().includes(searchUser.props.value.toLowerCase())
      );
      return temp;
    } catch (e) {
      console.log(e);
  ...
  return (
    <div className={cl.ProfilePage}>
      ...
      <DateInput
        type="transactions"
        updateFinishedOrders={getTransactions}
        dates={{
          yesterDay: localStorage.getItem("firstDateTr"),
          toDay: localStorage.getItem("secondDateTr"),
        }}
      ></DateInput>
      {isLoading ? (
      ...
      <Pagination
        totalGoods={transactions.length}
        paginate={paginate}
      ...
      {localStorage.getItem("role") === "1" ||
      localStorage.getItem("role") === "2" ? (
        <div className={cl.AdminTools}>
          <h1 className={cl.Title2}>Инструменты администратора</h1>
          <div className={cl.HorResizable}>
            ...
            <div className={cl.Select + " " + cl.Start}>
              <p className={cl.Center + " " + cl.FixedHeight}>
                Расчет c пользователем '{selectedUserName}', ID: {selectedUser}
              </p>
              <div className={cl.HorNoMargin}>
                <MyInput
                  {...sum.props}
                  placeholder="Сумма расчета"
                  inputMode={"number"}
                ></MyInput>
                <ErrorList>
                  {sum.props.isInvalid ? sum.errorText : ""}
                </ErrorList>
              </div>
              ...
              <div className={cl.Hor}>
                <Button
                  onClick={() => payOff("add")}
                  disabled={sum.valid || selectedUser === 0}
                >
                  {isPayOffLoading ? <Loading /> : "Добавить"}
                </Button>
                <Button
                  onClick={() => payOff("minus")}
                  disabled={sum.valid || selectedUser === 0}
                  color="green"
                >
                  {isPayOffLoading ? <Loading /> : "Расчет"}
                </Button>
              </div>
            </div>
          </div>
          <p className={cl.Center}>
            История транзакций пользователя '{selectedUserName}'
          </p>
          <DateInput
            type="transactionsOther"
            updateFinishedOrders={getTransactionsOther}
            dates={{
              yesterDay: localStorage.getItem("firstDateTrOther"),
              toDay: localStorage.getItem("secondDateTrOther"),
            }}
          ></DateInput>
          {isOtherLoading ? (
            <div className={cl.LoadingWrapper}>
          ...
          <Pagination
            totalGoods={transactionsOther.length}
            paginate={paginate2}
            goodsPerPage={goodsPerPage}
